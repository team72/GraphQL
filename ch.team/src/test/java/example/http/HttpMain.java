...
import example.graphql.StarWarsData;
...
public class HttpMain extends AbstractHandler {
	...
	private void handleStarWars(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		//
		// this builds out the parameters we need like the graphql query from
		// the http request
		...
		ExecutionInput.Builder executionInput = newExecutionInput().requestString(parameters.getQuery())
				.operationName(parameters.getOperationName()).arguments(parameters.getVariables());

		//
		// the context object is something that means something to down stream
		// code. It is instructions
		// from yourself to your other code such as DataFetchers. The engine
		// passes this on unchanged and
		// makes it available to inner code
		//
		// the graphql guidance says :
		//
		// - GraphQL should be placed after all authentication middleware, so
		// that you
		// - have access to the same session and user information you would in
		// your
	...
	private GraphQLSchema buildStarWarsSchema() {
		...
		// statically.
		//
		// A full application would use a dependency injection framework (like
		// Spring)
